...
def make_dataset(split: str, data_root: str, data_list_fpath: str) -> List[Tuple[str, str]]:
    ...
    image_label_list = []
    for line in lines:
        relative_paths = line.split()
        image_label_list.append(tuple([os.path.abspath(os.path.join(data_root, relative_paths[0])),
                                       os.path.abspath(os.path.join(data_root, relative_paths[1]))]))
